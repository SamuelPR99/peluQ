    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="mt-12">
            ...
            <form method="POST" action="{{ route('login') }}">
                ...
                <!-- Password -->
                <div class="mt-4">
                    <label for="password" class="block font-medium text-sm text-gray-300">{{ __('Contraseña') }}</label>
                    ...
                    @error('password')
                        <span class="text-white text-sm text-red-600 text-red-400 mt-2">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="flex items-center justify-end mt-4">
                    @if (Route::has('password.request'))
                        <a class="underline text-sm text-gray-400  hover:text-teal-600" href="{{ route('password.request') }}">
                            {{ __('¿No recuerdas la contraseña?') }}
                        </a>
                    @endif
    
                    <button type="submit" class="ms-3 inline-flex items-center px-4 py-2 bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-black  uppercase tracking-widest  hover:bg-gradient-to-r from-teal-600 to-lime-500 focus:bg-gray-700 hover:text-white active:bg-gray-900 active:bg-gray-300 focus:outline-none focus:ring-2 focus:border-teal-500 focus:ring-offset-2 focus:ring-offset-gray-800 transition ease-in-out duration-150">
                        {{ __('Iniciar Sesión') }}
                    </button>
                </div>
            </form>
